...
#收订
wechat_order_detail_create_sql= '''
...
from
     ...
     where data_date='{date}' and platform='4') t
left join(
select cust_id from (
    select
        cust_id
...
select
     ...
     round(case when t.out_profit is null then 0.00 else t.out_profit end,2) as out_profit,
     round(case when t.out_pay_amount is null then 0.00 else t.out_pay_amount end,2) as out_pay_amount,
