...
def get_resource_list():
    ...
    return each_resource_name_list
    pass
...
def download_audio():
    ...
    resource_name_list = get_resource_list()
    ...
    print("there are total " + str(len(resource_name_list)) + " 集")
    i = 1
    for resource_name, url in zip(resource_name_list, urllist):
        # cmd = "curl -s -o '" + resource_name.encode(
        #     'utf-8') + ".m4a' " + '-H "user-agent:' + useragent + '"' + ' -H "Host:' + host + '"' + " -b loginmobile.txt '" + url + "'"
        # ret = os.popen(cmd)
        r = requests.get(url, headers=headers, verify=False)
        with open('resource/'+resource_name+'.m4a', 'wb') as f:
            f.write(r.content)

        progressbar(i, len(urllist))
        i += 1
